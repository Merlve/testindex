...
- **Immersive Media Details & Direct Downloads**: Media details pages feature edge-to-edge backdrop hero images extending behind floating glass navigation bars, and instant direct episode/file downloads without opening unnecessary browser tabs.
- **Floating Navigation & Liquid Glass Controls**: Apple-inspired liquid glass pill bars (`backdrop-blur-xl`, `backdrop-saturate-[180%]`, specular rim light) with a persistent brand pill (SHUTTER! logo & text), a top-right action pill, and a floating search circle button positioned right above the scroll-to-top button on all devices.
...
- **Smart Scroll Restoration**: Intelligently records and restores exact scroll positions across history navigation, so returning from media details pages back to categories or search lists maintains your exact scroll location.
- **Instant Metadata Synchronization & Fixes**: Instant cache purging and state synchronization ensure TMDB metadata overrides and corrections take effect immediately across all client views and cached endpoints.
- **Enhanced Jellyfin Re-Fetch & Auto-Sync**: Active fetch button clears search caches and force-updates recent Jellyfin items in real-time, backed by an automated 3-minute background worker and client auto-refetch interval with robust clean-title scoring and fallback item generation.
